...
| Vocabulary | Apple contextual strings, Whisper prompt tokens, and Parakeet CTC rescoring only when its optional companion is installed. An explicit Parakeet download also downloads the CTC companion and shows the combined total before starting; if Parakeet is already installed, the first custom-vocabulary add triggers the companion download. Missing or failed CTC installation skips rescoring but never blocks the base Parakeet transcript. Settings reports companion installation/download progress and model-library errors. |
...
| Network | No telemetry, analytics, update checks, or background model downloads. Downloads are user-initiated: selecting Parakeet may include its CTC companion, and adding a custom term can fetch the companion when needed; the UI shows combined size before the Parakeet download. No speech, transcripts, corrections, filenames, or usage data are uploaded. |
