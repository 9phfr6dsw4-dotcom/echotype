...
| Text delivery | At the stop gesture, capture the currently frontmost app and supported text field, then paste there after transcription only if that same target is still focused. Reject secure fields, password-manager apps, changed focus, and unavailable Accessibility inspection; on rejection, keep the transcript and explain that it was not pasted. Optional Return is separately enabled. |
...
The project targets macOS 26 and requires Swift 6.0 or newer. CI must run on an Apple Silicon macOS runner and verify tests, build, package, extracted app, clean-location launch, and checksum. Real microphone and hotkey behavior still requires testing on a physical Mac.
...
[63] https://developer.apple.com/documentation/applicationservices/kaxsubroleattribute — Accessibility focused-element subrole attribute
[64] https://developer.apple.com/documentation/applicationservices/carbon_accessibility/subroles?changes=latest_bet_4&language=objc — Accessibility element subroles, including secure text fields
