...
- Delivery to the text field focused when you stop dictation. If its app does not expose that field through Accessibility, EchoType sends Command-V only while the same app remains frontmost; changed apps are not pasted into. Known secure fields and excluded password-manager apps remain blocked. Clipboard restoration is attempted by default, with optional keep-on-clipboard and Return submission.
